...
# v7: Focus-fire weakest enemy in multiplayer
FOCUS_FIRE_ENABLED = True
FOCUS_FIRE_WEAKEST_MULT = 1.35
FOCUS_FIRE_MIN_PLAYERS = 3

# v7: Production-deficit awareness
PROD_DEFICIT_HIGH_PROD_BONUS = 1.25
PROD_DEFICIT_THRESHOLD = 0.7

# v7: Proactive fleet interception
INTERCEPT_ENABLED = True
INTERCEPT_MIN_ENEMY_SHIPS = 8
INTERCEPT_MAX_ETA = 12
INTERCEPT_VALUE_MULT = 1.6
...
def safe_angle_and_distance(sx, sy, sr, tx, ty, tr):
    ...
    # Center blocked, try dodging by aiming at the edges of the planet
    d = dist(sx, sy, tx, ty)
    if d <= sr + tr + LAUNCH_CLEARANCE:
        return None
    
    # Calculate angular width of the planet from the source
    # We use a slightly smaller radius for safety when dodging
    safe_tr = tr * 0.9
    angle_to_center = math.atan2(ty - sy, tx - sx)
    try:
        angle_offset = math.asin(safe_tr / d)
    except ValueError:
        return None

    for sign in [1, -1]:
        test_angle = angle_to_center + sign * angle_offset
        start_x, start_y = launch_point(sx, sy, sr, test_angle)
        # Re-calculate hit distance for this specific angle
        # Law of cosines or simple projection
        test_hit_distance = math.sqrt(d*d - safe_tr*safe_tr) - (sr + LAUNCH_CLEARANCE)
        end_x = start_x + math.cos(test_angle) * test_hit_distance
        end_y = start_y + math.sin(test_angle) * test_hit_distance
        if not segment_hits_sun(start_x, start_y, end_x, end_y):
            return test_angle, test_hit_distance
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        # v7: identify weakest enemy for focus-fire
        enemy_strengths = {
            owner: strength
        ...
        if enemy_strengths:
            self.weakest_enemy = min(enemy_strengths, key=enemy_strengths.get)
            self.weakest_enemy_strength = enemy_strengths[self.weakest_enemy]
        else:
...
def opening_filter(target, arrival_turns, needed, src_available, world):
    ...
    # Option B: guaranteed first-mover — never block regardless of travel turns
    if reaction_gap >= SAFE_NEUTRAL_MARGIN * 2:
        return False
...
def target_value(target, arrival_turns, mission, world, modes):
    ...
    # Option C: gateway bonus — planets near clusters of capturable planets are more valuable
    nearby_capturable = sum(
        1 for other in world.neutral_planets + world.enemy_planets
        if other.id != target.id and dist(target.x, target.y, other.x, other.y) < 15
    )
    if nearby_capturable > 0:
        value *= (1.0 + 0.04 * nearby_capturable)

    # Improvement 3: extra urgency on enemy ships in very-late (every ship matters for final score)
    if world.is_very_late and target.owner not in (-1, world.player):
        value += max(0, target.ships) * 0.45

    # v7: Focus-fire weakest enemy in multiplayer — eliminate one player to simplify the game
    if (
        FOCUS_FIRE_ENABLED
        and world.num_players >= FOCUS_FIRE_MIN_PLAYERS
        and target.owner not in (-1, world.player)
        and world.weakest_enemy is not None
        and target.owner == world.weakest_enemy
    ):
        value *= FOCUS_FIRE_WEAKEST_MULT

    # v7: Production-deficit — when behind in prod, boost high-production targets
    if world.my_prod < world.enemy_prod * PROD_DEFICIT_THRESHOLD:
        if target.production >= 3:
            value *= PROD_DEFICIT_HIGH_PROD_BONUS
...
# Improvement 1: speed-send helper — tries larger fleets to exploit fleet-speed scaling
_SPEED_SEND_FRACS = [1.0, 1.4, 1.8]

def best_speed_send(src, target, needed, send_cap, world, modes, eff_turn_weight, mission_type="capture"):
    """Try 100/140/180% of needed ships; pick the send that maximises
    value/cost by exploiting the log-speed model (more ships → faster fleet)."""
    best = None
    for frac in _SPEED_SEND_FRACS:
        candidate = min(send_cap, max(needed, int(needed * frac)))
        if candidate < needed:
            continue
        aim = world.plan_shot(src.id, target.id, candidate)
        if aim is None:
            continue
        angle, turns, _, _ = aim
        value = target_value(target, turns, mission_type, world, modes)
        if value <= 0:
            continue
        score = apply_score_modifiers(
            value / (candidate + turns * eff_turn_weight + 1.0),
            target, mission_type, world,
        )
        if best is None or score > best[0]:
            best = (score, candidate, angle, turns)
    return best  # (score, send, angle, turns) or None


def plan_moves(world):
    modes = build_modes(world)

    # Improvement 2: dynamic turn weight — when behind in production be more aggressive,
    # when ahead be more conservative (scale ATTACK_COST_TURN_WEIGHT by prod ratio)
    prod_ratio = world.my_prod / max(1, world.enemy_prod)
    eff_turn_weight = ATTACK_COST_TURN_WEIGHT * max(0.75, min(1.20, prod_ratio))
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            # v7: Use best_speed_send to find optimal fleet size/speed
            speed_result = best_speed_send(
                src, target, rough_needed, src_available,
                world, modes, eff_turn_weight, "capture"
            )
            if speed_result is None:
                continue

            score, send_cap, angle, turns = speed_result
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                pair_score = apply_score_modifiers(
                    value / (need + joint_turn * eff_turn_weight + 1.0),
                    target, "swarm", world,
                )
    ...
    if not world.is_very_late:
        for src in world.my_planets:
            ...
            for target in world.planets:
                ...
                # Use best_speed_send to try larger fleets for speed advantage
                speed_result = best_speed_send(
                    src, target, rough_needed, min(src_left, send),
                    world, modes, eff_turn_weight, "capture",
                )
                if speed_result is not None:
                    sp_score, sp_send, _, _ = speed_result
                    if best is None or sp_score > best[0]:
                        best = (sp_score, target, sp_send)
                else:
                    value = target_value(target, est_turns, "capture", world, modes)
                    if value <= 0:
                        continue
                    score = apply_score_modifiers(
                        value / (send + est_turns * eff_turn_weight + 1.0),
    ...
    # v7: Proactive fleet interception — reinforce planets that enemy fleets are heading to
    if INTERCEPT_ENABLED:
        for planet in world.my_planets:
            timeline = world.base_timeline[planet.id]
            if timeline["first_enemy"] is None:
                continue
            enemy_eta = timeline["first_enemy"]
            if enemy_eta > INTERCEPT_MAX_ETA:
                continue
            # check if planet will hold without help
            if timeline["holds_full"]:
                continue
            # already committed reinforcement?
            if planned_commitments.get(planet.id):
                incoming = sum(
                    ships for _, owner, ships in planned_commitments[planet.id]
            ...
            for src in world.my_planets:
                if src.id == planet.id:
                    continue
                src_left = source_attack_left(src.id)
                if src_left < INTERCEPT_MIN_ENEMY_SHIPS:
                    continue
                aim = world.plan_shot(src.id, planet.id, src_left)
                if aim is None:
                    continue
                r_angle, r_turns, _, _ = aim
                if r_turns > enemy_eta:
                    continue
                need = world.reinforcement_needed_for(planet.id, r_turns, planned_commitments)
                if need <= 0:
                    continue
                send = min(src_left, need + 2)
                if send < need:
                    continue
                reinf_value = planet.production * max(1, world.remaining_steps - r_turns)
                reinf_value *= INTERCEPT_VALUE_MULT
                reinf_score = reinf_value / (send + r_turns * 0.4 + 1.0)
                if best_reinf is None or reinf_score > best_reinf[0]:
                    best_reinf = (reinf_score, src.id, r_angle, r_turns, send)
            if best_reinf is not None:
                _, r_src_id, r_angle, r_turns, r_send = best_reinf
                actual = append_move(r_src_id, r_angle, r_send)
                if actual >= 1:
                    planned_commitments[planet.id].append((r_turns, world.player, int(actual)))
    ...
    if world.doomed_candidates:
        ...
        for planet in world.my_planets:
            ...
            # Improvement 4: before blind retreat, try to join a planned swarm/mission
            joined_mission = False
            for cmt_target_id, commitments in planned_commitments.items():
                cmt_target = world.planet_by_id[cmt_target_id]
                if cmt_target.owner == world.player:
                    continue
                if not commitments:
                    continue
                cmt_eta = max(eta for eta, _, _ in commitments)
                still_needed = world.ships_needed_to_capture(cmt_target_id, cmt_eta, planned_commitments)
                if still_needed <= 0 or still_needed > available_now:
                    continue
                jm_aim = world.plan_shot(planet.id, cmt_target_id, still_needed)
                if jm_aim is None:
                    continue
                jm_angle, jm_turns, _, _ = jm_aim
                if jm_turns > world.remaining_steps - 2:
                    continue
                jm_actual = append_move(planet.id, jm_angle, still_needed)
                if jm_actual >= 1:
                    planned_commitments[cmt_target_id].append((jm_turns, world.player, int(jm_actual)))
                    joined_mission = True
                    break
            if joined_mission:
                continue
    ...
    # Option A: very-late consolidation — funnel ALL surplus ships to richest planet
    if world.is_very_late and len(world.my_planets) > 1:
        richest = max(
            world.my_planets,
            key=lambda p: p.ships + p.production * world.remaining_steps,
        )
        for src in world.my_planets:
            ...
            if surplus < 4:
                continue
            aim = world.plan_shot(src.id, richest.id, surplus)
            if aim is not None and aim[1] < world.remaining_steps - 1:
                append_move(src.id, aim[0], surplus)

    # Last-resort scatter: idle ships with nothing to do attack nearest enemy
    if world.enemy_planets and not world.is_very_late:
        for src in world.my_planets:
            idle = source_attack_left(src.id)
            if idle < 8:
                continue
            nearest_enemy = min(
                world.enemy_planets,
            ...
            aim = world.plan_shot(src.id, nearest_enemy.id, idle)
            if aim is None:
                continue
            aim_angle, aim_turns, _, _ = aim
            if aim_turns > world.remaining_steps - 3:
                continue
            append_move(src.id, aim_angle, idle)
...
def agent(obs):
    try:
        ...
        # Safety net: never crash on Kaggle — a crash = instant loss
        return []
